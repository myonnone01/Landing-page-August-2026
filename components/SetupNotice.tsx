...
export function SetupNotice({ missing }: { missing: string[] }) {
  return (
    <main className="mx-auto w-full max-w-2xl px-5 py-24 sm:px-8">
      ...
      <h1 className="mt-8 font-display text-3xl leading-tight font-bold tracking-tight text-sound-900 sm:text-4xl">
        The signup page isn&apos;t working right now.
      </h1>

      <p className="mt-5 text-[1.05rem] leading-relaxed text-sound-500">
        Sorry — this is on us, not you. The trip is still happening: {EVENT.boat}{" "}
        out of {EVENT.venue} on Tuesday, August 25, at the dock by 3:30 PM.
      </p>
      ...
      <p className="mt-4 text-[1.05rem] leading-relaxed text-sound-500">
        ...
        <a
          ...
          className="font-medium text-sound-900 underline decoration-shoal-400 underline-offset-4"
        >
      ...
      <div className="mt-12 rounded-lg bg-white px-5 py-5 ring-1 ring-shoal-400/25">
        <h2 className="font-mono text-[0.7rem] uppercase tracking-[0.18em] text-shoal-400">
          For whoever set this up
        </h2>
        {missing.length > 0 ? (
          <>
            <p className="mt-2.5 text-[0.9rem] leading-relaxed text-sound-500">
            ...
            <ul className="mt-3 space-y-1">
              {missing.map((name) => (
                <li
                  key={name}
                  className="font-mono text-[0.85rem] text-sound-900"
                >
                  {name}
                </li>
              ))}
          ...
          <p className="mt-2.5 text-[0.9rem] leading-relaxed text-sound-500">
            ...
            <code className="font-mono text-sound-900">DATABASE_URL</code> is
